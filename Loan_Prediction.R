...
pre
confusionMatrix(table(predict(glm,test),test$Loan_Status))  
library(pROC)
...
pre
confusionMatrix(table(predict(rpart,test),test$Loan_Status))  
library(pROC)
x=auc(test$Loan_Status,pre[,2])
x                               
plot(roc(test$Loan_Status,pre[,2]))
...
pre
confusionMatrix(table(predict(rf,test),test$Loan_Status))  
library(pROC)
x=auc(test$Loan_Status,pre[,2])
x                                   
plot(roc(test$Loan_Status,pre[,2]))  
